#[derive(Debug)]
pub struct Command {
    pub cmd_as_obj: CommandAsObject,
    pub cmd_as_str: String,
}

#[derive(Debug)]
pub enum CommandAsObject {
    ...
    fn make_cmd<'a>(&self, operation_type: OperationType, rest_of_iter: &mut impl Iterator<Item = &'a str>, full_str: &str) -> Command {
        Command {
            cmd_as_obj: match operation_type {
                ...
                OperationType::ArithmeticOperationType(op_type) => CommandAsObject::ArithmeticCommand { operationType: op_type },
            },
            cmd_as_str: full_str.to_string()
        }
    }
